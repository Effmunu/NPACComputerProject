                    ...
                    int binningEta, int binningPhi, int stained)
{
    ...
    TString outputPrefix = Form("%s_%s_%s_%d_%d_%s",
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++)
    {
        ...
        if (stained) {
            lambda0 = map.getIndex(ele0_eta, ele0_phi) % 2 == 0 ? 0.01 : -0.01;
            lambda1 = map.getIndex(ele1_eta, ele1_phi) % 2 == 0 ? 0.01 : -0.01;
        }
        ...
        info.phi0=ele0_phi;
        info.pt0=st_ele0_et;
        info.region0=map.getIndex(ele0_eta, ele0_phi);
        info.eta1=ele1_eta;
        info.phi1=ele1_phi;
        info.pt1=st_ele1_et;
        info.region1=map.getIndex(ele1_eta, ele1_phi);
        info.mass=mass;

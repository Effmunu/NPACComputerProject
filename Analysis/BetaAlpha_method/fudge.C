...
void fudge(string& categ, string& nbEvents, int binning)
{
    ...
    // read the alphas
    TString inputAplhas = inputPrefix + Form("_%d_unstained_alphas.txt",
                            binning);
    ...
    if (categ == "Z")
        input_mc_fitparam_string = Form("mc_%s_10000_%d_unstained_fitparam.txt",
                                categ.c_str(), binning); // Z
    else
        input_mc_fitparam_string = Form("mc_%s_50000_%d_unstained_fitparam.txt",
                                categ.c_str(), binning); // JPSI
    ...
    TString input_data_fitparam_string = Form("data_%s_%s_%d_unstained_fitparam.txt",
                        categ.c_str(), nbEvents.c_str(), binning);
    cout << "Will read data fit param from file: "
        << input_data_fitparam_string.Data() << endl;
